# Process raw eye-tracking data from Barcelona
...
# should missing samples be considered as FALSE looking?
non_aoi_as_na <- TRUE

# pre-process individual files -------------------------------------------------

gaze_files <- list.files(path = "data-raw/eyetracking-barcelona", 
						 pattern = "\\.csv",
						 full.names = TRUE)
...
for (i in 1:n_files){
	db[[i]] <- read_csv_arrow(gaze_files[i], na = na_strings) |>
		...
		mutate(phase = str_replace_all(phase, phase_dict),
...
gaze_processed <- processed |> 
	...
	mutate(x_imputed = na.locf(x, maxgap = 20, na.rm = FALSE),
...
# evaluate AOIs
participants <- arrow::read_csv_arrow(file.path("data/participants.csv"))
stimuli <- 
...
gaze_aoi <- gaze_processed |> 
	...
	mutate(is_gaze_prime = gaze_in_prime(x = x, 
										 y = y,
										 aoi_coords = aoi_coords),
		   is_gaze_target = gaze_in_target(x = x,
		   								y = y,
		   								target_location,
		   								aoi_coords),
		   is_gaze_distractor = gaze_in_distractor(x = x,
		   										y = y, 
		   										target_location,
		   										aoi_coords)) |> 
...
gaze_aoi <- x |> 
	...
	select(id, age, trial, phase, timestamp, x, y,
		   is_gaze_prime, is_gaze_target, is_gaze_distractor,
		   is_valid_gaze, is_imputed, trial_type, prime, target, distractor)
...
#' 
#' From 2022-05-26, the eye-tracker sampling rate was accidentally changed from 120Hz to 60Hz. This function makes sure that the processing of the datasets corresponding to subsequent experimental sessions takes this change in sampling rate  into account. 
#'
