...
try         % and enable a possible "clean" exit
    ...
    %%% INPUT GONZALO
    %% login prompt
    prompt         = {'file', 'sujenum','baby_id', 'date_test', 'name','date_birth', 'sex', 'language', 'list', 'comments'}; % define prompt variables
    %languagelist = {'Catalan','Spanish'};
    %[indx,tf] = listdlg('ListString',languagelist);
    promptDefaults = {'biprime', '000','00000', datestr(now, 'yyyy-mm-dd_HH-MM'),'','0000-00-00', '', '', '', ''}; % define prompt default values
    
    while 1
        promptAnswer   = inputdlg(prompt, 'BiPrime', 2, promptDefaults);                                                         % define prompt properties
        [output, sujenum, baby_id, date_test, name,date_birth, sex, language, list, comments] = deal(promptAnswer{:});           % all variables are strings
        
        if strcmp(language,'cat')==1 || strcmp(language,'spa')==1
            if strcmp(baby_id,'00000') == 0
        ...
        promptDefaults = {'biprime', '000',baby_id, datestr(now, 'yyyy-mm-dd_HH-MM'),'','0000-00-00', '', language, list, ''};
    end %while             
    
    filename       = [output sujenum '_' baby_id '_' name '_' date_test  '_birth_' date_birth '_' language list '.csv'];                               % output file name
    filepath       = cd;
    resFileName    = fullfile(RES_PATH, filename);
    
    %%%%%%%IF YOU WANT A DIFERENT FOLDER FOR EACH SUBJECT
    FolderName = [sujenum '_' baby_id '_' name];
    ...
    variableNames={'SystemTimeStamp','DeviceTimeStamp','meanX','meanY','meanValidity','lX','lY','lV','lPupil','lPupilV','rX','rY','rV','rPupil','rPupilV','sujeNUM','baby_id','date_birth','sex','testdate','language','list','numtrial_list','trial','trialtype','getternum','phase','stimulus','locationTARGET'};
    
    filenameLOG = [RES_PATH FolderName '\' output sujenum '_' baby_id '_' name '_' date_test '_' language '_' list 'LOG.txt'];
...
%         end
        ...
        while toc(tIni)<=PRIME_IMAGE_DURATION
            ...
            full_gaze_data = [full_gaze_data; parseGazeData(gaze_data, [sujenum,baby_id,date_birth,sex,date_test,language, list, num2str(num(ii,1)),itrial,num2str(num(ii,5)),num2str(num(ii,6)), "PRIMEIMAGE", lookText_DIS,'NA'])];
...
%         full_gaze_data_ALL = [full_gaze_data_ALL; full_gaze_data];
        ...
        while toc(tIni)<=BLANK_DURATION            
            ...
            full_gaze_data = [full_gaze_data; parseGazeData(gaze_data, [sujenum,baby_id,date_birth,sex,date_test,language, list, num2str(num(ii,1)),itrial,num2str(num(ii,5)),num2str(num(ii,6)), "BLANK", lookText_DIS,'NA'])];
        ...
        while toc(tIni)<=AUDIO_DURATION
            ...
            full_gaze_data = [full_gaze_data; parseGazeData(gaze_data, [sujenum,baby_id,date_birth,sex,date_test,language, list, num2str(num(ii,1)),itrial,num2str(num(ii,5)),num2str(num(ii,6)), "BLANK_AUDIO", lookText_DIS,'NA'])];
        ...
        while toc(tIni)<=TOTAL_DURATION
            ...
            %Screen('AsyncFlipEnd', monitorWindow);
            gaze_data = eyetracker.get_gaze_data();
            full_gaze_data = [full_gaze_data; parseGazeData(gaze_data, [sujenum,baby_id,date_birth,sex,date_test,language, list, num2str(num(ii,1)),itrial,num2str(num(ii,5)),num2str(num(ii,6)), "TARGET_DISTRACTOR", lookText_TARDIS,locationTAR])];
