...
```{r gaze_time_2125_mon_stringent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
augment(fit_2125_mon) %>% 
	...
	plot_annotation(
		caption = "A: Marginal means of the multilevel model, plotted in separate panels for 21- and 25-month olds.\nSample sizes are indicated at the upper left corner in each panel. The first and last 300 ms in each trial\nwere trimmed before the analysis. B: mean observed probability of target looking, aggregated by participant\nfirst and by prime type second.",
		tag_levels = "A") &
	theme_custom() &
	theme(
