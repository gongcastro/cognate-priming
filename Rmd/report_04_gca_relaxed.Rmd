...
draws %>% 
	...
	ggplot(aes(.value, .variable, fill = stat(cut_cdf_qi(cdf, labels = percent, .width = c(0.50, 0.80, 0.95, 1))))) +
...
vcov(fit) %>% 
	...
	geom_tile() +
	labs(x = "Term 1", y = "Term 2", fill = "Mean post. correlation") +
	...
	theme(
		...
		axis.title = element_blank(),
		axis.text = element_text(hjust = 1, size = 8),
...
ggplot(m, aes(time_bin_center, .value)) +
	...
	geom_text(data = sample_size, aes(x = min(gaze_relaxed$time_bin_center), y = 1, label = paste0("N = ", n)),
