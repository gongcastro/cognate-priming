...
tar_load(fit)
tar_load(fit_21_mon)
tar_load(fit_21_mon_vocab_split)
tar_load(fit_21_mon_vocab_balanced)
tar_load(fit_21_mon_location)

# relaxed version
tar_load(gaze_relaxed)
tar_load(attrition_relaxed)
tar_load(fit_relaxed)
tar_load(fit_21_mon_relaxed)
tar_load(fit_21_mon_vocab_split_relaxed)
tar_load(fit_21_mon_vocab_balanced_relaxed)
tar_load(fit_21_mon_location_relaxed)
...
attrition_location <- attrition %>% 
	left_join(select(participants_stringent, participant, location)) %>% 
...
left_join(attrition, select(participants_stringent, age_group, location, lp, participant, date_test)) %>% 
...
```{r fixed_effects_table_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
main_effects <- confint.merMod(fit_relaxed, method = "Wald") %>% 
	as.data.frame() %>% 
	clean_names() %>% 
	drop_na(x2_5_percent) %>%
	rownames_to_column("term") %>% 
	right_join(clean_names(tidy(fit_relaxed, effects = "fixed"))) %>% 
	select(term, estimate, std_error, conf_lower = x2_5_percent, conf_upper = x97_5_percent,
		   statistic, df, p_value) %>% 
	gt() %>% 
	fmt_number(2:7) %>% 
...
```{r random_effects_participant_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

VarCorr(fit_relaxed)$participant %>% 
	cov2cor() %>% 
	as.data.frame() %>% 
	rownames_to_column("term1") %>% 
	gt(rowname_col = "term1") %>% 
...
```{r gaze_time_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r gaze_time_vocab_lower_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
sample_size <- participants_relaxed %>% 
	count(age_group, lp)
...
```{r gaze_time_vocab_upper_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
sample_size <- participants_relaxed %>% 
	filter(vocab_size_l1_center > median(.$vocab_size_l1_center, na.rm = TRUE), valid_participant) %>% 
	count(age_group, lp)
...
```{r gaze_time_location_barcelona_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
sample_size <- participants_relaxed %>% 
	filter(location=="Barcelona", valid_participant) %>% 
	count(age_group, lp)
...
```{r gaze_time_location_oxford_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
sample_size <- participants_relaxed %>% 
	filter(location=="Oxford") %>% 
	count(age_group, lp)
...
```{r participants_cdi_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
participants_relaxed %>% 
	left_join(distinct(attrition_relaxed, participant, valid_participant)) %>%
	left_join(vocabulary) %>% 
...
```{r trials_attrition_location_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=8, out.width="100%"}
# plot participants
attrition_location <- attrition_relaxed %>% 
	left_join(select(participants_relaxed, participant, location)) %>% 
	count(location, lp, age_group, trial_type, valid_status) %>% 
	drop_na(trial_type)
...
```{r attrition_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=12}

left_join(attrition_relaxed, select(participants_relaxed, age_group, location, lp, participant, date_test)) %>% 
	group_by(participant, lp, location, trial_type, date_test, age_group, valid_participant) %>% 
	summarise(valid_trial_count = sum(valid_trial, na.rm = TRUE),
			  .groups = "drop") %>% 
	pivot_wider(
...
```{r participants_attrition_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(select(participants_relaxed, participant, location)) %>% 
	mutate(participant = str_remove(participant, "cognatepriming")) %>% 
...
```{r trials_attrition_table_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(select(participants_relaxed, participant, location)) %>% 
	group_by(location, lp, age_group, trial_type, valid_trial) %>% 
...
```{r trials_attrition_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(select(participants_relaxed, participant, location)) %>% 
	count(location, lp, age_group, trial_type, valid_status) %>% 
...
```{r attirion_summary_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, frequency_prime_childes)) %>% 
	drop_na(trial_type, frequency_prime_childes) %>% 
...
```{r attrition_frequency_target_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, frequency_target_childes)) %>% 
	drop_na(trial_type, frequency_target_childes) %>% 
	mutate(frequency_target_childes = round(frequency_target_childes)) %>% 
...
```{r attrition_familiarity_prime_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, familiarity_prime)) %>% 
	drop_na(trial_type, familiarity_prime) %>% 
...
```{r attrition_familiarity_target_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4, out.width="100%"}
# plot participants
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, familiarity_target)) %>% 
	drop_na(trial_type, familiarity_target) %>% 
...
```{r attrition_animacy_prime_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, is_animate_prime)) %>%
	mutate(is_animate_prime = ifelse(is_animate_prime, "Animate", "Inanimate")) %>% 
	drop_na(trial_type, is_animate_prime) %>% 
...
```{r attrition_animacy_target_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, is_animate_target)) %>%
	mutate(is_animate_target = ifelse(is_animate_target, "Animate", "Inanimate")) %>% 
	drop_na(trial_type, is_animate_target) %>% 
...
```{r attrition_category_prime_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, category_prime)) %>%
	drop_na(trial_type, category_prime) %>% 
	count(trial_type, age_group, valid_status, trial_type, category_prime) %>% 
...
```{r attrition_category_target_relaxed, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
attrition_relaxed %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, category_target)) %>%
	drop_na(trial_type, category_target) %>% 
	count(trial_type, age_group, valid_status, trial_type, category_target) %>% 
...
```{r vocabulary_l1_stringent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4}
sample_size <- participants_stringent %>% 
...
```{r vocabulary_total_stringent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4}
sample_size <- participants_stringent %>% 
	filter(valid_participant, !is.na(vocab_size_l1)) %>% 
	count(age_group, location)
...
```{r vocabulary_conceptual_stringent, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=4}
sample_size <- participants_stringent %>% 
	filter(valid_participant, !is.na(vocab_size_l1)) %>% 
	count(age_group, location)
