...
participants %>% 
	...
	mutate(lp = ifelse(lp=="Monolingual", "Mon.", "Bil.")) %>% 
	ggplot(aes(lp, fill = valid_participant, colour = valid_participant)) +
=======
## Sample sizes
...
<<<<<<< HEAD
We collected data from `r sum(duplicated(participant$participant)` longitudinal participants, that is, who were tested in at least two of the three ages (21, 25, and 30). Of those, `r sum(duplicated(participant$participant[participants$location=="Oxford"])` were tested in Oxford, and `r `sum(duplicated(participants$participant[participants$location=="Barcelona"])` were tested in Barcelona.
...
participants %>% 
	drop_na(valid_participant) %>% 
	ggplot(aes(age_group, participant, fill = valid_participant, colour = valid_participant)) +
...
<<<<<<< HEAD
attrition_summary %>% 
	mutate(valid_trial = ifelse(valid_participant, "Valid", "Invalid"),
		   participant = paste0(participant, " (", percent(missing_prop), ")")) %>% 
	ggplot(aes(missing_prop, participant, colour = valid_participant, fill = valid_participant)) +
	geom_col() +
	...
	ggsave(here("Figures", "missing.png"), height = 15)
=======
attrition %>%
...
lp_labels <- c("Monolingual", "Bilingual")
names(lp_labels) <- lp_labels
age_group_labels <- c("21 months", "25 months")
names(age_group_labels) <- c("21", "25")

gaze_time %>% 
	filter(valid_participant, valid_trial, age_group %in% c(21, 25),
		   location == "Barcelona") %>% 
	group_by(time_bin, trial_type, lp, age_group,location) %>% 
=======
...
gaze_time_vocab_d %>% 
	...
	ggplot(aes(x = time_bin, y = prop, colour = trial_type, fill = trial_type)) +
<<<<<<< HEAD
	facet_grid(lp~age_group, labeller = labeller(lp = lp_labels, age_group = age_group_labels)) +
=======
...
<<<<<<< HEAD
	...
	labs(x = "Time (ms)", y = "Pr(Target fixation)", colour = "Condition", fill = "Condition") +
=======
	#geom_point(alpha = 0.5) +
	labs(x = "Time bin (100 ms)", y = "Fixations", colour = "Condition", fill = "Condition") +
>>>>>>> 4dd17d331bc163cb69b583725cc7efb451529267
...
<<<<<<< HEAD
	...
	ggsave(here("Figures", "gaze_time.png"), height = 5, width = 7)
=======
		  panel.grid = element_line(colour = "grey", linetype = "dotted"))
>>>>>>> 4dd17d331bc163cb69b583725cc7efb451529267
