...
## Stringent criteria  {.tabset .tab-pills}
...
## Relaxed criteria  {.tabset .tab-pills}
...
left_join(attrition_relaxed, select(participants, age_group, location, lp, participant, date_test)) %>% 
	group_by(participant, lp, location, trial_type, date_test, age_group, valid_participant) %>% 
	summarise(valid_trial_count = sum(valid_trial, na.rm = TRUE),
			  .groups = "drop") %>% 
	pivot_wider(
