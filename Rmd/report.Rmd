...
options(knitr.kable.NA = '-',
		ggplot2.discrete.fill = ggthemes::colorblind_pal()(8),
		ggplot2.discrete.colour = ggthemes::colorblind_pal()(8),
		ggplot2.continuous.fill = ggthemes::colorblind_pal()(8),
		ggplot2.continuous.colour = ggthemes::colorblind_pal()(8)
)
...
sample_size <- participants %>% 
	filter(vocab_size_center <= median(.$vocab_size_center, na.rm = TRUE), valid_participant) %>% 
	count(age_group, lp)

augment(fit) %>% 
	mutate(.fitted_prob = logit_to_prob(.fitted)) %>% 
	filter(vocab_size_center <= median(.$vocab_size_center)) %>% 
...
sample_size <- participants %>% 
	...
	filter(valid_participant, age_group=="21 months", lp=="Monolingual") %>% 
	mutate(vocab_cat = ifelse(vocab_size_center > median(participants$vocab_size_center, na.rm = TRUE), "Above median", "Below median")) %>% 
...
sample_size <- participants %>%
	...
	arrange(location, desc(vocab_size)) %>% 
	group_by(location) %>% 
...
participants %>% 
	...
	geom_violin(position = position_dodge(width = 0.75),
				aes(lp, vocab_size, colour = lp, fill = lp)) + 
	# geom_point(shape = 1, colour = "black", alpha = 0.5, size = 3, stroke = 1,
	# 		   position = position_jitter(width = 0.1, seed = 888)) +
	geom_boxplot(aes(lp, vocab_size, colour = lp, fill = lp),
				 width = 0.1, colour = "black", fill = "white", outlier.colour = NA,
