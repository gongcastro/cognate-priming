...
gaze_trial %>% 
	...
	drop_na() %>% 
	ggplot(aes(aoi, prop, colour = aoi, fill = aoi)) +
...
gaze_time_vocab_d %>% 
	...
	#geom_ribbon(aes(ymin = lower, ymax = upper), width = 0) 
	#geom_point(alpha = 0.5) +
	labs(x = "Time bin (100 ms)", y = "Fixations", colour = "Vocab-AOI",
		 fill = "Vocab-AOI", shape = "Vocab-AOI") +
	scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
	scale_colour_brewer(palette = "Set1") +
	scale_fill_brewer(palette = "Set1") +
	theme_bilingual() +
...
gaze_time_vocab_d %>% 
	...
	#geom_ribbon(aes(ymin = lower, ymax = upper), width = 0) 
	#geom_point(alpha = 0.5) +
	labs(x = "Time bin (100 ms)", y = "Fixations", colour = "Location-AOI",
