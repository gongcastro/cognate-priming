...
```{r stimuli_frequency_prime, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, fig.height=5, out.width="80%"}

stimuli %>% 
	distinct(location, test_language, target, prime, .keep_all = TRUE) %>% 
	ggplot(aes(trial_type, frequency_prime, colour = trial_type, fill = trial_type)) +
...
```{r stimuli_frequency_target, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, fig.height=5, out.width="80%"}

stimuli %>% 
	distinct(location, test_language, target, prime, .keep_all = TRUE) %>% 
	ggplot(aes(trial_type, frequency_target, colour = trial_type, fill = trial_type)) +
	facet_wrap(location~test_language) +
	geom_violin() +
	geom_boxplot(colour = "black", fill = "white", outlier.colour = NA, width = 0.1) +
	geom_jitter(colour = "black", size = 1, shape = 1,
...
```{r stimuli_familiarity_prime, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, fig.height=5, out.width="100%"}
means <- stimuli %>% 
...
animacy %>% 
	...
	facet_wrap(role~location~test_language, ncol = 4) +
...
stimuli %>% 
	...
	labs(x = "% of trials", y = "Trial type",
		 colour = "Trial type", fill = "Trial type") +
	scale_fill_manual(values = wesanderson::wes_palette(name = "Zissou1", n = 9, type = "continuous")) +
	scale_colour_manual(values = wesanderson::wes_palette(name = "Zissou1", n = 9, type = "continuous")) +
	scale_x_continuous(labels = scales::percent) +
	theme(
		legend.position = "top",
		legend.title = element_blank(),
...
augment(fit) %>% 
	...
	ggsave(here("Figures", "emmeans.png"), height = 6)
```
...
# plot participants
attrition %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, frequency_prime)) %>% 
	drop_na(trial_type, frequency_prime) %>% 
	mutate(frequency_prime = round(frequency_prime)) %>% 
	count(trial_type, age_group, valid_status, trial_type, frequency_prime) %>% 
	ggplot(aes(as.factor(frequency_prime), n, fill = valid_status, colour = valid_status)) +
...
attrition %>% 
	left_join(distinct(stimuli, test_language, location, list, trial, trial_type, frequency_target)) %>% 
	drop_na(trial_type, frequency_target) %>% 
	mutate(frequency_target = round(frequency_target)) %>% 
	count(trial_type, age_group, valid_status, trial_type, frequency_target) %>% 
	ggplot(aes(as.factor(frequency_target), n, fill = valid_status, colour = valid_status)) +
	facet_grid(trial_type~age_group) +
	geom_col(position = position_fill(), colour = "white") +
	labs(x = "Trial type", y = "Trials", colour = "Validity", fill = "Validity") +
	guides(fill = guide_legend(ncol = 2), colour = guide_legend(ncol = 2))	+
...
augment(fit_21_mon) %>% 
	...
	ggsave(here("Figures", "emmeans_21_mon.png"))
```
...
augment(fit_21_mon_location) %>% 
	...
	ggsave(here("Figures", "emmeans_21_mon_location.png"))
```
...
augment(fit_21_mon_vocab) %>% 
	...
	ggsave(here("Figures", "emmeans_21_mon_vocab.png"))
```
...
augment(fit_21_mon_vocab_pct) %>% 
	...
	ggsave(here("Figures", "emmeans_21_mon_vocab_pct.png"))
```
...
participants %>% 
	...
	ggsave(here("Figures", "vocabulary_l1.png"))
...
participants %>% 
	...
	ggsave(here("Figures", "vocabulary_total.png"))
...
participants %>% 
	...
	ggsave(here("Figures", "vocabulary_conceptual.png"))
