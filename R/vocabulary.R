...
get_vocabulary_bcn <- function(
  ...
  out <- bvq_data$vocabulary |>
    ...
    right_join(
      select(participants_tmp, session_id, child_id, vocab_id, age, lp),
      by = join_by(vocab_id)
    ) |>
    rowwise() |>
    ...
    left_join(
      select(participants_tmp, child_id, session_id, vocab_id, age, lp),
    ...
    impute_vocabulary(
      cols_impute = c("l1_count", "l2_count"),
      cols_predictor = c("age", "lp"),
      bvq_data = bvq_data,
      constraints = c(0, 371)
    ) |>
    impute_vocabulary(
      cols_impute = "concept_count",
      cols_predictor = c("age", "lp"),
      bvq_data = bvq_data,
      constraints = c(0, 375)
    ) |>
    impute_vocabulary(
      cols_impute = "te_count",
      cols_predictor = c("age", "lp"),
      bvq_data = bvq_data,
      constraints = c(0, 340)
    ) |>
    impute_vocabulary(
      cols_impute = "total_count",
      cols_predictor = c("age", "lp"),
      bvq_data = bvq_data,
      constraints = c(0, 715)
    ) |>
    ...
    mutate(
      contents = if_else(
        add_vocab_supp & !is.na(contents_supp),
        contents_supp,
        contents
      )
    ) |>
...
get_vocabulary_supp_bcn <- function(vocabulary_supp_bcn_file) {
  out <- vocabulary_supp_bcn_file |>
    ...
    map_dfr(
      function(x) {
        read_csv(
          ...
          pivot_longer(-x, names_to = "session_id", values_to = "response") |>
          mutate(
    ...
    summarise(contents_supp = list(item[response]), .by = session_id)

  return(out)
}

#' Impute vocabulary size based on the whole BVQ database
impute_vocabulary <- function(
  x,
  cols_impute,
  cols_predictor,
  bvq_data,
  ...
  if (!is.null(constraints)) {
    post[cols_impute] <- paste0(
      "imp[[j]][,i] <- squeeze(imp[[j]][, i], c(",
      constraints[1],
      ", ",
      constraints[2],
      "))"
...
get_vocabulary_oxf <- function(vocabulary_file, participants, words) {
  ...
  cdi_full <- get_cdi_full_oxf(vocabulary_file) |>
    ...
    rename(vocab_id = unique_id, vocab_id_response = response_id) |>
    summarise(
      l1_count = sum(response, na.rm = TRUE),
      l1_prop = mean(response, na.rm = TRUE),
      contents = list(item[response]),
      .by = c(vocab_id, vocab_id_response)
    ) |>
    left_join(
      select(participants_tmp, session_id, vocab_id),
      by = join_by(vocab_id)
    ) |>
    drop_na(session_id) |>
  ...
  cdi_extended <- get_cdi_extended_oxf(vocabulary_file) |>
    ...
    left_join(
      select(participants_tmp, session_id, vocab_id_response),
...
get_cdi_full_oxf <- function(vocabulary_file) {
  ...
  out <- readxl::read_xlsx(
    ...
    mutate(
      across(sounds_baa_baa_sheep:quantifiers_some, recode_responses),
      across(c(premature_birth, hearing_problems), \(x) {
        !grepl("no", tolower(x))
      }),
    ...
    mutate(
      semantic_category = str_extract(
        item,
        paste0(rev(category_names), collapse = "|")
      ),
      item = gsub(
        paste0(paste0(rev(category_names), "_"), collapse = "|"),
        "",
        item
      )
    )
...
get_cdi_extended_oxf <- function(vocabulary_file) {
  ...
  out <- readxl::read_xlsx(
    ...
    mutate(
      across(animal_sounds_baa_baa:online_youtube, recode_responses),
      across(c(premature_birth, hearing_problems), \(x) {
        !grepl("no", tolower(x))
      })
    ) |>
    filter(finished) |>
    select(response_id, recorded_date, animal_sounds_baa_baa:online_youtube) |>
    drop_na(response_id) |>
    pivot_longer(
    ...
    mutate(
      semantic_category = str_extract(
        item,
        paste0(rev(category_names_2), collapse = "|")
      ),
      item = gsub(
        paste0(paste0(rev(category_names_2), "_"), collapse = "|"),
        "",
        item
      )
    )
...
get_supplementary_oxf <- function(vocabulary_file) {
  out <- readxl::read_xlsx(
    ...
    pivot_longer(-participant_id, names_to = "item", values_to = "response") |>
