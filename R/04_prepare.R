...
trials <- read_xlsx(here("Stimuli", "stimuli.xlsx")) %>%
	...
	select(TrialID, TrialIDLabel, Location, Language, List)
...
# create time polynomials
t <- poly((unique(data$TimeBin)), 3)
data[, paste("TimeBin", 1:3, sep="")] <- t[data$TimeBin, 1:3]

#### reorder variables ###################################
data <- data %>%
	select(ParticipantID, TrialID = TrialIDLabel, starts_with("Time"),
...
logs.trials <- data %>%
	group_by(TrialID, TrialType, Language, LangProfile) %>%
	summarise(
		TimeBinN    = n(),
		TimeBinMin  = min(TimeBin, na.rm = TRUE),
