...
#' 
save_files <- function(x, folder, file_name = deparse(substitute(x)),
                       formats = c("csv", "rds"), .sep = "/") {
    # check arguments
    ...
    if (length(formats) > 1) {
        ...
        if (any(!dirs_exist)) {
            ...
            missing_dir <- glue("{folder}{.sep}{which_missing}{.sep}")
...
get_vars_dict <- function(responses) {
    ...
    var.labels <- c(
        ...
        "Comprehension",
        glue("Length (+1 SD, {round(sd(responses$n_phon), 2)} phonemes)"),
        glue("Age (+1 SD, {round(sd(responses$age), 2)} months)"),
        glue("Exposure (+1 SD, {round(sd(responses$exposure), 2)})"),
        glue("Cognateness (+1 SD, {round(sd(responses$lv), 2)})"),
...
get_vars_dict_doe <- function(responses) {
    ...
    var.labels <- c(
        ...
        "Comprehension",
        glue("Age (+1 SD, {round(sd(responses$age), 2)}, months)"),
        glue("Phonemes (+1 SD, {round(sd(responses$n_phon), 2)} phonemes)"),
        glue("Frequency (+1 SD, {round(sd(responses$freq), 2)})"),
        glue("DoE (+1 SD, {round(sd(responses$doe), 2)})"),
        glue("Cognateness (+1 SD, {round(sd(responses$lv), 2)})"),
        "Age \u00d7 DoE",
        "DoE \u00d7 Cognateness",
        "Age \u00d7 Cognateness",
