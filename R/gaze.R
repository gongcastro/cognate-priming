...
get_gaze_bcn <- function(
  ...
  out <- processed |>
    ...
    mutate(
      is_gaze_prime = gaze_in_prime(x, y, aoi_coords),
...
get_gaze_processed_bcn <- function(files, res = c(x = 1920, y = 1080)) {
  ...
  out <- out |>
    ...
    mutate(
      is_valid_gaze = if_else(is.na(is_valid_gaze), FALSE, is_valid_gaze),
...
make_non_aoi_as_false <- function(x, non_aoi_as_na = FALSE) {
  if (non_aoi_as_na) {
    na_sub <- list(
      ...
      is_gaze_distractor = FALSE
    )
    x <- tidyr::replace_na(x, na_sub)
  }
  return(x)
}
...
get_gaze_oxf <- function(files, participants, stimuli) {
  ...
  d_s <- stimuli |>
    filter(test_language == "English") |>
    distinct(trial_type, prime, target, distractor, test_language, version)
