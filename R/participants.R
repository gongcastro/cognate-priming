...
#'
get_participants <- function(participants_file_bcn, participants_file_oxf) {
  lp_levels <- c("Monolingual (English)", "Monolingual", "Bilingual")
  lang_levels <- c("Catalan", "English", "Spanish")
  sex_levels <- c("f", "m")

  participants_bcn <- read_csv(
  ...
  participants_oxf <- participants_file_oxf |>
    ...
    mutate(
      across(starts_with("date_"), \(x) suppressWarnings(dmy(x))),
      across(matches("_id"), as.character),
      age_group = paste0(age_group, " months"),
      age_group = as.factor(age_group),
      vocab_id_response = if_else(
        grepl("^R_", vocab_id),
        vocab_id,
        NA_character_
      ),
      session_id = child_id,
      child_id = if_else(
        !is.na(previous_id_if_applicable),
  ...
  out <- list(Barcelona = participants_bcn, Oxford = participants_oxf) |>
