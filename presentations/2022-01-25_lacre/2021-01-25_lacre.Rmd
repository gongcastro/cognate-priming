...
n <- expand_grid(
	...
	vocab_size_total_center = c(-1, 0, 1)
)

m <- epred_draws(object = fit_total, newdata = n, ndraws = 20, re_formula = NA) %>% 
	mutate(.value = logit_to_prob(.epred))

ggplot(m, aes(time_bin_center, .value)) +
	facet_wrap(vocab_size_total_center ~ fct_rev(lp)) +
