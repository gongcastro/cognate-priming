make_plots <- function() {
  dirs <- file.path("assets", "gaze", c("time", "heatmap", "aggregated"))
  for (i in dirs) {
    if (!dir.exists(i)) {
...
make_plots_time <- function(
  ...
  cli_progress_bar("Plotting", total = n_files, format = pb_msg)
  ...
  for (i in 1:n_files) {
    ...
    ggsave(
      file.path(paste0("assets/gaze/time/", file_name)),
      plot = plot,
      height = plot_height,
      width = 6
    )
...
get_plot_data <- function(
  ...
  gaze_tmp <- gaze |>
    ...
    left_join(
      attrition_trials_tmp,
      by = join_by(session_id, trial, trial_type)
    ) |>
  ...
  plot_data <- gaze_tmp |>
    mutate(
      target_y = if_else(
        target_location == "l",
        mean(c(aoi_coords$l[["xmin"]], aoi_coords$l[["xmax"]])),
        mean(c(aoi_coords$r[["xmin"]], aoi_coords$r[["xmax"]]))
      ),
      target_y = if_else(phase == "Prime", NA, target_y),
      id_age = paste0(session_id, " (", round(age, 2), ")"),
      is_phase_valid = if_else(
        phase == "Prime",
...
plot_gaze_data <- function(x, file) {
  ...
  plot <- x |>
    ...
    geom_blank(
      data = dummy_data,
      aes(xmin = 0, xmax = 2, ymin = ymin, ymax = ymax),
      inherit.aes = FALSE
    ) +
    geom_vline(xintercept = 0.3, linewidth = 0.25, colour = "grey") +
    geom_rug(
      data = filter(x, is_gaze_target, phase == "Target"),
      length = unit(0.05, "cm"),
      sides = "t",
      colour = "mediumspringgreen"
    ) +
    geom_rug(
      data = filter(x, is_gaze_distractor, phase == "Target"),
      length = unit(0.05, "cm"),
      sides = "b",
      colour = "mediumspringgreen"
    ) +
    ...
    geom_text(
      aes(x = 0, y = 0, label = paste0(is_phase_valid, ", ", is_valid_trial)),
      colour = "black",
      size = 1.5,
      vjust = 0,
      hjust = 0
    ...
    scale_x_continuous(
      ...
      breaks = seq(0, 2, 0.5),
      labels = function(x) {
        format(x * 1000, big.mark = ",", scientific = FALSE)
      },
      sec.axis = dup_axis()
    ) +
...
make_plots_heatmap <- function(
  ...
  plot_data <- get_plot_data(gaze_aoi, attrition_trials, attrition_participants)
  ...
  for (i in 1:n_files) {
    ...
    ggsave(
      paste0("assets/gaze/01-heatmap/", file_name),
      plot = plot,
      height = plot_height,
      width = 8,
      dpi = 800
...
plot_heatmap_data <- function(plot_data, file) {
  ...
  plot <- plot_data |>
    ...
    ggplot(aes(x = x, y = y, colour = timestamp)) +
    facet_wrap(
      trial ~ phase,
      ncol = 8,
      labeller = label_wrap_gen(multi_line = FALSE)
    ...
    geom_text(
      aes(
        x = 1920,
        y = 0,
        label = paste0(is_phase_valid, ", ", is_valid_participant)
      ),
      colour = "black",
      size = 1.5,
      vjust = 0,
    ...
    geom_text(
      data = right_join(
        aois,
        distinct(plot_data, trial, phase, target_location, phase),
...
make_plots_aggregated <- function(
  gaze,
  participants,
  stimuli,
  vocabulary,
  ...
  plot_data <- data_time |>
    left_join(attrition_participants, by = join_by(id, age_group)) |>
    mutate(
      is_valid_participant = factor(
        is_valid_participant,
        levels = c(FALSE, TRUE),
  ...
  for (i in 1:n_participants) {
    plot <- plot_data[[i]] |>
      ...
      geom_hline(yintercept = 0.5, colour = "grey", linewidth = 1) +
      stat_summary(fun.data = mean_se, geom = "line", linewidth = 3 / 4) +
    ...
    ggsave(
      paste0("assets/gaze/02-processed/", file_name),
      plot = plot,
      height = 5,
