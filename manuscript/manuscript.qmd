...
loos_03 <- loo_compare(model_loos_bcn[1:4]) |> 
	as.data.frame() |> 
	rownames_to_column("model") |> 
	mutate(model = paste0("Model ", as.numeric(gsub("fit_", "", model))),
		   across(matches("diff"),
		   	   \(x) {x[1] <- NA; return(x)}))

loos_36 <- loo_compare(model_loos_bcn[4:7]) |> 
	as.data.frame() |> 
	rownames_to_column("model") |> 
	mutate(model = paste0("Model ", as.numeric(gsub("fit_", "", model))),
		   across(matches("diff"),
...
bind_rows("Base models" = loos_03,
		  "Age and vocabulary size models" = loos_36, 
		  .id = "type") |> 
	select(type, model, matches("elpd|diff")) |> 
	relocate(matches("diff"), .after = matches("loo")) |> 
	gt(rowname_col = "model", groupname_col = "type") |> 
	fmt_number(is.numeric) |> 
	...
	tab_footnote(footnote = md("*ELPD*: sum of expected log pointwise predictive density for a new dataset. *SE \\(ELPD\\)*: standard error of the *ELPD*, which indictes the uncertainty about the predictive performance for unknown future data. *ELPD (diff)*: pairwise difference in *ELPD* for two models. The difference is computed relative to the model with highest *ELPD*. *SE ELPD (diff.)*: standard error of component-wise differences of *ELPD* between two models.")) |> 
...
A comparison between models including *Age* ($\mathcal{M}_{4}$), *L1 vocabulary* ($\mathcal{M}_{5}$), and *Total vocabulary* ($\mathcal{M}_{6}$)---and their three-way and two-way interactions with *Group* and *Condition*---against model $\mathcal{M}_{3}$, which only included *Age* as a covariate is shown in @tbl-loos. Overall, all models performed equivalently, with the simplest model $\mathcal{M}_{3}$ showing slightly better performance. This suggests that participants' target looking during the test phase can be predicted with relative accuracy without taking into account *L1 vocabulary*, or *Total vocabulary* sizes.
...
loos_03 <- loo_compare(model_loos_oxf[1:2]) |> 
	as.data.frame() |> 
	rownames_to_column("model") |> 
	mutate(model = paste0("Model ", as.numeric(gsub("fit_oxf_", "", model))),
		   across(matches("diff"),
		   	   \(x) {x[1] <- NA; return(x)}))

loos_36 <- loo_compare(model_loos_oxf[2:4]) |> 
	as.data.frame() |> 
	rownames_to_column("model") |> 
	mutate(model = paste0("Model ", as.numeric(gsub("fit_oxf_", "", model))),
		   across(matches("diff"),
