...
ggsave(file.path("img", "vocabulary.jpeg"),
	   dpi = 800, height = 8, height = 4)
```
...
data_time |> 
	...
	ggplot(aes(timebin, .prop, colour = lp)) +
...
<<<<<<< HEAD
```{r fig-ptlt-recognition-age}
#| label: fig-ptlt-recognition-age
#| fig-cap: "Average probability of target looking across the time course of the trial. Points an whiskers indicate the mean and standard error of the mean."
#| fig-width: 11
#| fig-height: 4
data_time |> 
	summarise(.prop = mean(.prop),
			  .by = c(lp, timebin, age_group, session_id)) |> 
	ggplot(aes(timebin, .prop, colour = age_group)) +
	facet_wrap(~lp, nrow = 1, scales = "free_x") +
...
ggsave(file.path("img", "recognition-age.jpeg"),
	   height = 4, width = 11,
	   dpi = 800)
=======
```{r tbl-coefs}
post <- gather_draws(fit_0, `b_.*`, regex = TRUE)  

ggplot(post, aes(.value)) +
	facet_wrap(~.variable, scales = "free") +
	geom_histogram(bins = 15, colour = "white") +
	geom_errorbar(data = mean_qi(post),
				  aes(xmin = .lower, xmax = .upper, y = 400),
				  width = 1,
				  linewidth = 3/4) +
...
ggsave(file.path("img", "model-agg.jpeg"),
	   height = 7.5, width = 10,
	   dpi = 800)
```
...
nd <- expand_grid(condition = levels(fit_0$data$condition),
				  timebin_std = timebin_values,
				  age_std = age_values,
				  # sesion_id = first(data$session_id),
				  # age = c(21, 25, 30),
				  lp = levels(fit_0$data$lp)) 
