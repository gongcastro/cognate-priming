...
To test cross-language priming effects, we registered participants' time course of target picture looking in the three conditions. We predicted within-language priming effects, revealed as differences in target looking between *Unrelated* and *Related/Non-cognate* trials, as previously reported [@von2012, @MANI2001, and @MANI2011]. We further predicted cross-language priming effects, revealed as differences in target looking between *Related/Non-cognate* and *Related/Cognate* trials. 
...
Overall, participants' looking time exceeded chance levels, as indicated by the fact that the 95% HDI of the intercept term excluded zero ($\beta$ = `r coefs$fit_4$b_intercept$mdn_label`, 95% HDI = `r coefs$fit_4$b_intercept$ci_label`). The variability of participants' target preference varied significantly more between participants ($\sigma$ = `r coefs$fit_4$sd_child_id_intercept$mdn_label`, 95% HDI = `r coefs$fit_4$sd_child_id_intercept$ci_label`) than within testing sessions from the same participant ($\sigma$ = `r coefs$fit_4$sd_child_id_session_id_intercept$mdn_label`, 95% HDI = `r coefs$fit_4$sd_child_id_session_id_intercept$ci_label`). The 95% HDI of the coefficient of *Age* had a positive sign, but did not exclude zero ($\beta$ = `r coefs$fit_4$b_age_std$mdn_label`, 95% HDI = `r coefs$fit_4$b_age_std$ci_label`), indicating that participants from all ages showed equivalent overall target word recognition.

The 95% HDI of the coefficient of *Group* in $\mathcal{M}_{1}$ included zero ($\beta$ = `r coefs$fit_4$b_lp1$mdn_label`, 95% HDI = `r coefs$fit_4$b_lp1$ci_label`), indicating an equivalent overall target preference in monolinguals and bilinguals. The 95% HDI of the first contrast of this predictor, comparing *Unrelated* and *Related/Non-cognate* trials, included zero ($\beta$ = `r coefs$fit_4$b_condition1$mdn_label`, 95% HDI = `r coefs$fit_4$b_condition1$ci_label`). The 95% HDI of the second contrast, comparing *Related/Non-cognate* and *Related/Cognate* trials, also included zero ($\beta$ = `r coefs$fit_4$b_condition2$mdn_label`, 95% HDI = `r coefs$fit_4$b_condition2$ci_label`). The overall target preference was equivalent across both pairwise condition comparisons. The interaction term between the first *Condition* contrast contained zero ($\beta$ = `r coefs$fit_4$b_condition1_lp1$mdn_label`, 95% HDI = `r coefs$fit_4$b_condition1_lp1$ci_label`). The interaction term between the second *Condition* contrast also contained zero ($\beta$ = `r coefs$fit_4$b_condition2_lp1$mdn_label`, 95% HDI = `r coefs$fit_4$b_condition2_lp1$ci_label`). The outcomes of this model indicate that lack of differences in overall target looking time across conditions remained across monolinguals and bilinguals.
...
doe_mon_exp <- participants_bcn |>
	...
	count(is_positive) 
```
Both @mani2010infant and @mani2011phonological, and the present study shared a similar design, but sampled different children populations. Participants in Mani and Plunkett's studies were raised in English speaking households in Oxford, while participants in the present study were learning Catalan or Spanish, if not a combination of both. We identify two ways in which this might have influenced the results. One is that the stimuli lists in Catalan and Spanish might have differed in some critical dimension, which interfered infants' ability to generate implicit labels for prime pictures. For example, words in the present study were predominantly bi-syllabic, while those from Mani and Plunkett's studies were mostly monosyllabic, and contained less phonemes on average. Longer word-forms might have been more difficult to retrieve in time for a prime effect to take place in target recognition. Another possibility is that exposure to a second language might have delayed the developmental trajectories of word production in bilinguals, in such way implicit naming emerges later ages. Given that `r scales::percent(doe_mon_exp$n[1] / doe_mon_exp$n[2], accuracy = 0.01)`
...
```{r tbl-loos-oxf}
loos_tbl |> 
	mutate(model = gsub("fit_oxf_", "", model)) |>
	gt() |> 
	fmt_number(is.numeric) |> 
	# fmt_markdown(is.character, ) |> 
	cols_label(model = "Model",
...
@tbl-loos-oxf summarises the outputs of the LOO-CV model comparisons. Models $\mathcal{M}_{1}$, $\mathcal{M}_{2}$, and $\mathcal{M}_{3}$ showed substantially better predicted performance than model $\mathcal{M}_{0}$, which indicates that adding the *Condition* predictor improves significantly the fit of the models. All $\mathcal{M}_{1}$, $\mathcal{M}_{2}$, and $\mathcal{M}_{3}$ models showed equivalent performance, suggesting that adding the two-way interaction between *Age* and *Condition* did not improve the the fit of the model. We now report a description of the posterior distribution of the regression coefficients of model $\mathcal{M}_{3}$.

Overall, participants' looking time exceeded chance levels, as indicated by the fact that the 95% HDI of the intercept term excluded zero ($\beta$ = `r coefs$fit_3$b_intercept$mdn_label`, 95% HDI = `r coefs$fit_3$b_intercept$ci_label`). The variability of participants' target preference varied significantly more between participants ($\sigma$ = `r coefs$fit_3$sd_child_id_intercept$mdn_label`, 95% HDI = `r coefs$fit_3$sd_child_id_intercept$ci_label`) than within testing sessions from the same participant ($\sigma$ = `r coefs$fit_3$sd_child_id_session_id_intercept$mdn_label`, 95% HDI = `r coefs$fit_3$sd_child_id_session_id_intercept$ci_label`). The 95% HDI of the coefficient of *Age* had a positive sign, but did not exclude zero ($\beta$ = `r coefs$fit_3$b_age_std$mdn_label`, 95% HDI = `r coefs$fit_0$b_age_std$ci_label`), indicating that participants from all ages showed equivalent overall target word recognition.

The 95% HDI of the first contrast of the *Condition* predictor, comparing *Unrelated* and *Related/Non-cognate* trials, included zero ($\beta$ = `r coefs$fit_3$b_condition1$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition1$ci_label`). The 95% HDI of the second contrast, comparing *Related/Non-cognate* and *Related/Cognate* trials, also included zero ($\beta$ = `r coefs$fit_3$b_condition2$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition2$ci_label`). The overall target preference was equivalent across both pairwise condition comparisons. The interaction term between the first *Condition* contrast contained zero ($\beta$ = `r coefs$fit_3$b_condition1_lp1$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition1_lp1$ci_label`). The interaction term between the second *Condition* contrast also contained zero ($\beta$ = `r coefs$fit_3$b_condition2_lp1$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition2_lp1$ci_label`). Both younger and older participants showed equivalent looking time across conditions, as suggested by the fact that the 95% HDI of the two-way interaction between *Age* and the first *Condition* contrast included zero ($\beta$ = `r coefs$fit_3$b_condition1_age_std$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition1_age_std$ci_label`), as well as the interaction between *Age* and the second *Condition* contrast ($\beta$ = `r coefs$fit_3$b_condition2_age_std$mdn_label`, 95% HDI = `r coefs$fit_3$b_condition2_age_std$ci_label`).
...
Another possibility is that the findings in @mani2010infant and @mani2011phonological are less robust than anticipated.
