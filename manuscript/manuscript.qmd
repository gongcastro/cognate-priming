...
participants_bcn |>
    ...
    summary_rows(
        groups = c("Monolingual", "Bilingual"),
        columns = matches("age"),
        fns = list(Total ~ mean(.)),
...
loo_compare(model_loos_oxf[2:4]) |>
    ...
    mutate(
        model = paste0("Model ", as.numeric(gsub("fit_oxf_", "", model)) - 1),
        across(
            matches("diff"),
            \(x) {
                x[1] <- NA
...
epreds_chance_report <- epreds_chance |>
    mutate(
        over_chance = sprintf("%.2f", over_chance),
        age_std = factor(
            x = age_std,
            levels = age_values,
...
stimuli |>
    ...
    summarise(
        across(
            c(matches("n_phon|freq_|lv_|nphon"), duration),
            list(
