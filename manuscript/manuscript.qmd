...
attrition_trials_vtarget <- attrition_trials_vtarget |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_participants_vnone <- attrition_participants_vnone |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_trials_vnone <- attrition_trials_vnone |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_participants_noeach <- attrition_participants_noeach |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_trials_noeach <- attrition_trials_noeach |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_participants_vnoeach <- attrition_participants_vnoeach |> 
	left_join(select(participants, id, age_group, lp, filename))

attrition_trials_vnoeach <- attrition_trials_vnoeach |> 
	left_join(select(participants, id, age_group, lp, filename))
...
participants_oxf |> 
	left_join(attrition_participants_oxf,
			  by = join_by(id)) |> 
	mutate(lp = "Monolingual (English)",
		   id = as.character(id)) |> 
	bind_rows(mutate(participants, id = as.character(id))) |> 
	filter(is_valid_participant) |> 
	select(id, age_group, age, lp, test_language) |> 
	add_count(lp, 
			  name = "n_lp") |> 
	add_count(age_group, 
			  name = "n_age_group") |> 
	...
	pivot_wider(id_cols = c(test_language, lp),
				...
				values_from = c(matches("doe"), age_mean, age_sd, n),
...
* [@floccia2020translation] requires participants to know prime and target, but not to look at target *and* distractor (looking at at least one of them is enough).
...
tibble::lst(data_time_related,
	...
	pivot_wider(names_from = c(lp, condition),
				values_from = .ntrials,
				names_repair = janitor::make_clean_names) |> 
	relocate(matches("oxford"), matches("barcelona")) |> 
...
epreds <- expand_grid(condition = levels(data_time_related$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_related[[1]],
					ndraws = NULL,
					re_formula = NA, 
					value = ".value") |> 
...
epreds <- expand_grid(condition = levels(data_time_cognate$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_cognate[[1]],
					ndraws = NULL,
					re_formula = NA) |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_related_vtarget$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_related_vtarget[[1]],
					ndraws = NULL,
					re_formula = NA, 
					value = ".value") |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_cognate_vtarget$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_cognate_vtarget[[1]],
					ndraws = NULL,
					re_formula = NA) |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_related_vnone$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_related_vnone[[1]],
					ndraws = NULL,
					re_formula = NA, 
					value = ".value") |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_cognate_vnone$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_cognate_vnone[[1]],
					ndraws = NULL,
					re_formula = NA) |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_related_noeach$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_related_noeach[[1]],
					ndraws = NULL,
					re_formula = NA, 
					value = ".value") |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
epreds <- expand_grid(condition = levels(data_time_cognate_noeach$condition),
					  ...
					  .nsamples = 1) |>
	add_epred_draws(model_fits_cognate_noeach[[1]],
					ndraws = NULL,
					re_formula = NA) |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
#| tbl-cap: "Trial attrition rate by condition for included participants. Additional excluded trials are indicated between parentheses."
attrition_trials_vnoeach |> 
	filter(id %in% attrition_participants_vnoeach$id[attrition_participants_vnoeach$is_valid_participant]) |> 
	left_join(select(participants, filename, id, age_group),
			  by = join_by(filename, id, age_group)) |> 
	summarise(n_valid = sum(is_valid_trial),
			  n_total = n(),
...
#| fig-width: 8
#| fig-cap: "Marginal posterior predictions of the GAMMs. (A) Mean posterior probability of target looking across the time course of the trial. Black lines and intervals indicate the psoterior mean and 95% credible intervals. Points indicate the mean probability of target looking across participants. (B) Difference in posterior probability of target looking between *related* and *unrelated* trials. The yellow rectangle indicates, in both A and B, the range of time points in which the 95% credible interval of the differences excluded zero."
epreds <- expand_grid(condition = levels(data_time_related_vnoeach$condition),
					  timebin = seq(0, 17, length.out = 100),
					  age = mean(data_time_related_vnoeach$age),
					  lp = levels(data_time_related_vnoeach$lp),
					  .nsamples = 1) |>
	add_epred_draws(model_fits_related_vnoeach[[1]],
					ndraws = NULL,
					re_formula = NA, 
					value = ".value") |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
...
#| fig-width: 8
#| fig-cap: "Marginal posterior predictions of the GAMMs. (A) Mean posterior probability of target looking across the time course of the trial. Black lines and intervals indicate the psoterior mean and 95% credible intervals. Points indicate the mean probability of target looking across participants. (B) Difference in posterior probability of target looking between *cognate* and *non-cognate* trials. The yellow rectangle indicates, in both A and B, the range of time points in which the 95% credible interval of the differences excluded zero."
epreds <- expand_grid(condition = levels(data_time_cognate_vnoeach$condition),
					  timebin = seq(0, 17, length.out = 100),
					  age = mean(data_time_cognate_vnoeach$age),
					  lp = levels(data_time_cognate_vnoeach$lp),
					  .nsamples = 1) |>
	add_epred_draws(model_fits_cognate_vnoeach[[1]],
					ndraws = NULL,
					re_formula = NA) |> 
	mutate(lp = factor(lp, levels = c("Monolingual", "Bilingual")))
