...
### Phonological priming: Related vs. Unrelated primes
...
nd <- expand(data_related, lp, condition, age = mean(data_related$age))

epreds <- add_epred_draws(newdata = nd,
						  object = model_fits_related[[1]],
						  re_formula = NA) |> 
	mean_qi() |> 
...
epreds |> 
	...
	# 		  colour = "grey",
	# 		  alpha = 1/4) +
	geom_dots(dat = data_related,
			  aes(y = .elog,
			  	side = ifelse(condition=="Related",
			  				  "left", 
			  				  "right")),
			  show.legend = FALSE) +
	geom_hline(yintercept = 0,
			   linetype = "dashed") +
	geom_line(aes(group = lp),
			  colour = "black",
			  linewidth = 3/4) +
...
plot_gamm <- function(data, model, contrast = "related") {
	...
	data |> 
		...
		summarise(.prop = mean(.prop),
				  .by = c(child_id, timebin, lp, condition, age)) 
	...
	epreds |> 
		ggplot(aes(timebin, y = .value,
				   ...
				   linetype = condition)) +
		facet_grid(age~lp) +
		stat_summary(fun.data = \(x) mean_qi(x, .width = 0.95),
					 geom = "ribbon",
					 alpha = 0.5,
					 linewidth = 0) +
...
```{r fig-cognate-aggregated}
#| label: fig-related-aggregated
#| fig-cap: "A) Aggregated probability of target looking for cognate and non-cognate trials. Each point represents the mean probability of target looking for one participant in one testing session. Lines link observations from the same testing session across conditions. Black dots and errorbars indicate the mean and 95% CrI. B) Aggregated probability of target looking across ages for cognate and non-cognate trials. Lines and error bands indicate the mean and standard error of the mean."
nd <- expand(data_cognate, lp, condition, age = mean(data_cognate$age))

epreds <- add_epred_draws(newdata = nd,
						  object = model_fits_cognate[[1]],
						  re_formula = NA) |> 
	mean_qi() |> 
	mutate(label = sprintf("%.2f\n[%.2f, %.2f]", .epred, .lower, .upper))
...
epreds |> 
	ggplot(aes(condition, .epred, 
			   ...
			   fill = condition)) +
	facet_wrap(~lp) +
	# geom_line(data = data_cognate,
	# 		  aes(y = .elog,
	# 		  	group = session_id),
	# 		  colour = "grey",
	# 		  alpha = 1/4) +
	geom_dots(dat = data_cognate,
			  aes(y = .elog,
			  	side = ifelse(condition=="Cognate",
			  				  "left", 
			  				  "right")),
			  show.legend = FALSE) +
	geom_hline(yintercept = 0,
...
plot_gamm(data_time_cognate,
		  model_fits_cognate[[1]],
		  contrast = "cognate")
```
