...
%     t7700 = (ceil(7.7 / monitorFlipInterval) - 0.5) * monitorFlipInterval; % 7700 ms
    ...
    % LIST
    [num, words]     = xlsread([listsPath 'list_' language '\' version '\' 'stimuli_' language list '.xlsx']); % read list
    ...
    for iTrial = 1:nTrials % for each trial until nTtrials
        ...
        targetAudioPath     = [audiosPath   char(words(ii, 5))]; % get the ii-th target audio
        targetLocation      = char(words(ii, 4));                       % get the ii-th target locatiion
