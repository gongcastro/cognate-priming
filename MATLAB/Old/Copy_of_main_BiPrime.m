...
try         % and enable a possible "clean" exit
    ...
    % set paths GONZALO ------------------------------------------------------------
    mainDirectory = 'C:\Users\cbclab\Documents\gGarciaCastro\BiPrime';
    cd(mainDirectory);
    ...
    audiosPath  = [mainDirectory '\sounds\'];
    ...
    STIM_PATH                 = strcat(mainDirectory, filesep, 'Stim', filesep);
    RES_PATH                  = strcat(mainDirectory, filesep, 'Res', filesep);
    %GETTER_DURATION          = 3;
    ...
    promptDefaults = {'biprime' '000', '', '', '0000-00-00', '', '', '', ''}; % define prompt default values
    ...
    variableNames={'SystemTimeStamp', 'DeviceTimeStamp', 'meanX', 'meanY', 'meanValidity', 'lX', 'lY', 'lV', 'lPupil', 'lPupilV', 'rX', 'rY', 'rV', 'rPupil', 'rPupilV', ...
...
% do some stuff
    ...
    writetable(array2table(fullGazeData, 'VariableNames', variableNames), gazeDataPath);
    Screen('CloseAll');
    fprintf(logDataPath,'END: %s\n', datestr(now, 'dd/mm/yyyy HH:MM:SS'));
    fclose(logDataPath);
    fclose(fid);
