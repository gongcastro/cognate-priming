...
try         % and enable a possible "clean" exit
    ...
    %%% INPUT GONZALO
    %% login prompt
    prompt         = {'Experiment','Participant','ID','Participant name','Birthdate','Sex','Language','List','Comments'}; % define prompt variables
    ...
    while 1
        ...
        [output,participant,id,name,dateBirth,sex,language,list,comments] = deal(promptAnswer{:}); % all variables are strings
        if strcmp(language,'cat')==1 || strcmp(language,'spa')==1
            if strcmp(id,'00000') == 0
                if strcmp(list,'1')==1 || strcmp(list,'2')==1 || strcmp(list,'3')==1
                    break;
                else
                    errordlg('Please specify \bf list (1, 2, or 3)', 'Unspecified field'); % throw error message
                end
            else
                errordlg('Please specify variable \bf ID (e.g. 55694)', 'Unspecified field'); % throw error message
            end
        else
            errordlg('Please specify testing \bf language (spa/cat)', 'Unspecified field'); % throw error message
        end
        promptDefaults = {'CognatePriming', '000','','','0000-00-00', '', '', '', ''}; % define prompt default values
    end %while
    ...
    baselinePath  = [allImagesPath 'baseline_' language '\'];
    ...
    variableNames={'SystemTimeStamp', 'meanX', 'meanY', 'meanValidity', 'lX', 'lY', 'lV', 'lPupil', 'lPupilV', 'rX', 'rY', 'rV', 'rPupil', 'rPupilV', ...
        'participant', 'dateBirth', 'sex', 'dateTest', 'language', 'list', ...
        'trial', 'trialID', 'trialType', 'getter', 'phase', 'stimulus', 'locationTarget'};
...
%     end
    ...
    if keyCode == Y_KEY
        fprintf(logDataPath,'POSITION: %s\n', datestr(now, 'dd/mm/yyyy HH:MM:SS'));
    ...
    keyCode = waitKeyPress();
    if keyCode == Y_KEY
        fprintf(logDataPath,'Calibration time: %s\n', datestr(now, 'dd/mm/yyyy HH:MM:SS'));
...
%     t7700 = (ceil(7.7 / monitorFlipInterval) - 0.5) * monitorFlipInterval; % 7700 ms
    ...
    % LIST
    [num, words]     = xlsread([listsPath 'list_' language list '.xlsx']); % read list
    ...
    for iTrial = 1:nTrials % for each trial until nTtrials
        ...
        % for the i-th  trial -------------------------------------------------
        baselineImagePath   = [baselinePath char(words(ii, 1)) '.jpg']; % get the ii-th prime image
...
%             end
        ...
        %%% [LOAD] BASELINE IMAGE
        imData               = imread(baselineImagePath);                       % read image file and make it a matrix
        [iy, ix, id]         = size(imData);                                    % measure the matrix
        iRectBaseline        = [0 0 ix iy];                                     % resize the matrix
        textureIndexBaseline = Screen('MakeTexture', eyeTrackerWindow, imData); % define matrix as an image
        %textureIndexPrime   = Screen('MakeTexture', monitorWindow, imData);   % define matrix as an image
        disp(['[LOAD ' num2str(iTrial) '] Baseline image loaded']);
...
%         end
       ...
       %% [INI] BASELINE IMAGE
        disp(['[INI ' num2str(iTrial) '] BASELINE IMAGE']);
        ...
        while toc(tIni)<=baselineImageDuration
            ...
            fullGazeData  = [fullGazeData; parseGazeData(gazeData, [participant,dateBirth,sex,dateTest,language,list,iTrial,num2str(num(ii,1)),words(ii,4),num2str(num(ii,6)),'baseline',lookTextPrime,'center'])];
            ...
            if find(keyCode, 1) == E_KEY                
                throw(MException('Prime: Warning', 'End by experimenter: Inside Baseline'))                
            end
            
            % MONITOR WINDOW            
            Screen('DrawTexture', monitorWindow, textureIndexBaseline, iRectBaseline,...  % prepare the image to be presented
                [xCenter/2-(ix/4) yCenter/2-(iy/4) xCenter/2+(ix/4) yCenter/2+(iy/4)]);
            % Screen('DrawDots', windowPtr, xy [,size] [,color] [,center] [,dot_type][, lenient]);
            Screen('DrawDots', monitorWindow, [x y]/SCREEN_MONITOR_PROPORTION, 40, EYE_DOT_COLOR, [], 2);
              
            % EYETRACKER WINDOW
            Screen('DrawTexture', eyeTrackerWindow, textureIndexBaseline, iRectBaseline,...  % prepare the image to be presented
                [xCenter-(ix/2) yCenter-(iy/2) xCenter+(ix/2) yCenter+(iy/2)]);
            if DRAW_CUES
                Screen('DrawDots', eyeTrackerWindow, [x y], 40, EYE_DOT_COLOR, [], 2);
            end
...
%         fullGazeData_ALL = [fullGazeData_ALL; fullGazeData];
        ...
        fiBaseline     = toc(tIni);
        nFlipsBaseline = nFlips;
        nFlips      = 0;
        %fprintf(logDataPath,'PRIME nFlips %d SECONDS %8.2f\t\n',nFlips,fiPrime);
        disp(['[END ' num2str(iTrial) '] BASELINE IMAGE']);
        ...
        while toc(tIni)<=primeImageDuration
            ...
            fullGazeData  = [fullGazeData; parseGazeData(gazeData, [participant,dateBirth,sex,dateTest,language,list,iTrial,num2str(num(ii,1)),words(ii,4),num2str(num(ii,6)),'prime',lookTextPrime,'center'])];
...
%         fullGazeData_ALL = [fullGazeData_ALL; fullGazeData];
        ...
        while toc(tIni)<=blankDuration            
            ...
            fullGazeData  = [fullGazeData; parseGazeData(gazeData, [participant,dateBirth,sex,dateTest,language,list,iTrial,num2str(num(ii,1)),words(ii,4),num2str(num(ii,6)),'blank',lookTextBlank,'center'])];
        ...
        nFlipsBlank = nFlips;
        nFlips      = 0;
        %fprintf(logDataPath,'BLANK nFlips %d SECONDS %8.2f\t\n',nFlips,fiBlank);
        %fullGazeData_ALL = [fullGazeData_ALL; fullGazeData];
        ...
        while toc(tIni)<=audioDuration
            ...
            %Screen('AsyncFlipEnd', monitorWindow);
            gazeData = eyetracker.get_gaze_data();
            fullGazeData  = [fullGazeData; parseGazeData(gazeData, [participant,dateBirth,sex,dateTest,language,list,iTrial,num2str(num(ii,1)),words(ii,4),num2str(num(ii,6)),'audio',lookTextBlank,'center'])];
        ...
        nFlipsAudio = nFlips;
        nFlips      = 0;
        %fprintf(logDataPath,'AUDIO nFlips %d SECONDS %8.2f\t\n',nFlips,fiAudio);
        %fullGazeData_ALL = [fullGazeData_ALL; fullGazeData];
        ...
        while toc(tIni)<=totalDuration
            ...
            %Screen('AsyncFlipEnd', monitorWindow);
            gazeData = eyetracker.get_gaze_data();
            fullGazeData  = [fullGazeData; parseGazeData(gazeData, [participant,dateBirth,sex,dateTest,language,list,iTrial,num2str(num(ii,1)),words(ii,4),num2str(num(ii,6)),'target_distractor',lookTextTargetDistractor,locationTarget])];
        ...
        nFlips                 = 0;        
        %fprintf(logDataPath,'TARDIS nFlips %d SECONDS %8.2f\t\n',nFlips,fiTargetDistractor);
        ...
        fprintf(logDataPath,'GETTER %8.2f\t PRIME-FLIPS %d\t PRIME-SEC %8.2f\t BLANK-FLIPS %d\t BLANK-SEC %8.2f\t AUDIO-FLIPS %d\t AUDIO-SEC %8.2f\t TARDIS-FLIPS %d\t TARDIS-SEC %8.2f\t\n',endGetter,nFlipsPrime,fiPrime,nFlipsBlank,fiBlank,nFlipsAudio,fiAudio,nFlipsTargetDistractor,fiTargetDistractor);
    ...
    promptAnswer2 = inputdlg(prompt2, 'BiPrime', 1, promptDefaults2); % define prompt properties
    [additionalComments] = deal(promptAnswer2{:}); % all variables are strings
    
    fprintf(logDataPath, 'Comments: %s\n',additionalComments);
    
    disp('[END] Additional comments');
    
    Screen('CloseAll');
    fprintf(logDataPath,'END: %s\s', datestr(now, 'dd/mm/yyyy HH:MM:SS'));
    fclose('all');
