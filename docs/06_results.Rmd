...
corr_mat <- VarCorr(fit_2, summary = TRUE)$participant$cor 
corr_names <- rownames(VarCorr(fit_2, summary = TRUE)$participant$cor)
...
m %>% 
	...
	scale_fill_d3() +
	scale_x_continuous(
		breaks = seq(min(gaze$time_bin_center), max(gaze$time_bin_center), len = 5),
		labels = format(100*(seq(0, 20, 5)), big.mark = ",")
	) +
	scale_y_continuous(
		limits = c(0, 1),
		labels = percent
...
ggsave(here("img", "predictions.png"))
...
m <- emmeans(fit_2, ~trial_type | lp, epred = TRUE) 
