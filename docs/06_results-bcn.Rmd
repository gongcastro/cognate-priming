...
model_comparison <- waics_l1 %>% 
	...
	mutate(model = str_replace_all(model, c("fit_l1_4" = "L1 vocab.", "fit_total_4" = "Total vocab."))) %>% 
	gt() %>% 
	fmt_number(-model) %>% 
...
corr_mat <- VarCorr(fit_total_4, summary = TRUE)$participant$cor 
corr_names <- rownames(VarCorr(fit_total_4, summary = TRUE)$participant$cor)
...
corr_mat <- VarCorr(fit_total_4, summary = TRUE)$target$cor 
corr_names <- rownames(VarCorr(fit_total_4, summary = TRUE)$target$cor)
...
m <- epred_draws(object = fit_prior_l1_prior, newdata = n, ndraws = 50, re_formula = NA) %>% 
	mutate(
...
m <- epred_draws(object = fit_total_4, newdata = n, ndraws = 50, re_formula = NA) %>% 
	mutate(
		.value = logit_to_prob(.epred),
		vocab_size_total_center = ifelse(
			vocab_size_total_center==-1, 
...
m <- emmeans(fit_total_4, ~trial_type | lp, epred = TRUE) 
...
ggsave(here("img", "epreds_l1.pdf"), dpi = 1000)
