...
draws <- gather_draws(model_fits$fit_bcn, `b_.*`, `sigma`, regex = TRUE)
...
m <- emmeans(model_fits$fit_bcn, ~trial_type:lp, by = "age_group", epred = TRUE) %>% 
	as_tibble() %>% 
	clean_names() %>% 
...
m <- epred_draws(object = model_fits$fit_bcn, newdata = n, ndraws = 50, re_formula = NA) %>% 
