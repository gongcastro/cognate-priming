...
suppressWarnings({
  time_diffs <- participants_oxf |>
    ...
    filter(
      child_id %in%
        participants_oxf$child_id[duplicated(participants_oxf$child_id)]
    ) |>
    pivot_wider(
      id_cols = child_id,
...
#| tbl-cap: "Demographic and linguistic profile of testing sessions. The number of excluded testing sessions and participants is indicated between parentheses."
participants |>
  unnest_wider(doe) |>
  ...
  mutate(
    study = if_else(location == "Oxford", "Study 1", "Study 2"),
...
vocab_plot_data |>
  filter(study == "Study 1 (English)") |>
  ggplot(aes(age, l1_count, side = side)) +
  geom_point(
    size = 2,
    shape = 1,
    colour = "grey",
...
attrition |>
  ...
  cols_merge(
    c(n_trials_true_cognate, n_trials_false_cognate),
    pattern = "{1} ({2})"
  ) |>
  cols_merge(
    c(n_trials_true_non_cognate, n_trials_false_non_cognate),
    pattern = "{1} ({2})"
  ) |>
  cols_merge(
    c(n_trials_true_unrelated, n_trials_false_unrelated),
    pattern = "{1} ({2})"
  ) |>
  cols_label(
    true = "Participants",
...
nd <- expand_grid(
  distinct(data_oxf, condition),
  timebin_std = timebin_values,
  age_std = 0
)

epreds <- add_epred_draws(
  nd,
  model_fits_oxf$fit_oxf_1,
  re_formula = NA,
  ndraws = NULL,
  value = ".epred"
)
...
plot_epreds +
  ...
  scale_x_continuous(
    ...
    labels = \(x) format(seq(3e2, 2e3, 3e2), big.mark = ",")
  ) &
  theme(
    legend.position = "top",
    legend.title = element_blank()
...
loo_compare(model_loos_oxf) |>
  ...
  tab_footnote(
    footnote = md(
      "*ELPD*: sum of expected log pointwise predictive density for a new data set. *SE \\(ELPD\\)*: standard error of the *ELPD*, which indictes the uncertainty about the predictive performance for unknown future data. *ELPD (diff)*: pairwise difference in *ELPD* for two models. The difference is computed relative to the model with lowest *ELPD* (best fitting model). *SE ELPD (diff.)*: standard error of component-wise differences of *ELPD* between two models."
    )
  ) |>
...
suppressWarnings({
  time_diffs <- participants_bcn |>
    ...
    filter(
      child_id %in%
        participants_bcn$child_id[duplicated(participants_bcn$child_id)]
    ) |>
    pivot_wider(
      id_cols = child_id,
      names_from = session_n,
...
vocabulary_bcn <- vocabulary |>
  inner_join(
    select(participants_bcn, session_id, lp, age_group),
    by = join_by(session_id)
  )

vocab_means <- vocabulary_bcn |>
  summarise(
    across(c(total_prop, l1_prop), lst(mean, sd)),
    n = n(),
    .by = c(age_group, lp)
  ) %>%
...
vocab_plot_data |>
  ...
  filter(study == "Study 2 (Catalan/Spanish)") |>
...
Participants were presented with 32 trials in random order, which belonged to three conditions: *Unrelated* trials (*n* = 16), *Related/Non-cognate* (*n* = 8), and *Related/Cognate* (*n* = 8). In *Unrelated* trials, the target label shared phonological onset with the Catalan and Spanish labels of the prime picture (e.g., prime: /ˈgos/~CAT~--/ˈpe.ro/~SPA~ [dog], target: /ˈka.za/~CAT~ [house], for a child tested in Catalan). In *Related/Non-cognate* trials, the target shared phonological onset with the prime label in the test language, but not with the prime label in the other language (e.g., prime: /miˈd͡ʒo/~CAT~ (/kal.θeˈtin/~SPA~) [dog], target: /muˈnɛ.ðə/~CAT~ [coin]). In *Related/Cognate* trials, the target shared phonological overlap with both English and Spanish prime labels (e.g., prime: /ˈa.βɾə/~CAT~--/ˈaɾ.bol/~SPA~) [tree], target: /əˈβɛ.ʎə/~CAT~ [bee]).

### Stimuli

We created six stimuli lists: three in Catalan, and three in Spanish. Lists were created following the same constraints as in Study 1,  but now considering the cross-linguistic phonological relationship between Catalan and Spanish. Extracting lexical frequencies from the Catalan and Spanish corpora in the CHILDES database was not possible, given the low number of participants and tokens included. We mapped the English lexical frequencies onto their Catalan and Spanish translation equivalents [see @fourtassi2020growth and @garcia2025cognate for similar approaches]. The auditory stimuli were natural exemplars of the selected target words, spoken by a proficient female bilingual speaker of Catalan (Central variety) and Castilian Spanish, who was instructed to pronounce each word in a toddler-directed manner. Catalan audios had an average duration of `r durations$catalan$duration_mean` ms (*SD* = `r durations$catalan$duration_sd`, *Range* = `r durations$catalan$duration_min`--`r durations$catalan$duration_max`), and Spanish audios had an average duration of  `r durations$spanish$duration_mean` ms (*SD* = `r durations$spanish$duration_sd`, *Range* = `r durations$spanish$duration_min`--`r durations$spanish$duration_max`). New visual stimuli were created to accommodate the words included in the new stimuli lists, and possible cultural differences in the typicality of the exemplars shown in the pictures (see @sec-appendix-d for a detailed description of the stimuli).


### Procedure

Same as in Study 1. We run the study on Windows 10 64-bit, using custom Matlab (2018a 64-bit) script using the PsychToolbox-3 extension (3.0.15 64-bit) to present the stimuli on a 23-inches screen with 1929 $\times$ 1080 resolution, and the Tobii Analytics SDK 3.0 to interact with the eye-tracker (Tobii TX300 and Tobii Pro Sprectrum, Tobii Technology, Stockholm, Sweden) while the experiment was running.
...
n_sessions_valid <- sum(
  distinct(attrition_bcn, session_id, .keep_all = TRUE)$is_valid_participant
)
n_participants_valid <- n_distinct(
...
n_exc_participants_vocab <- sum(
  !distinct(attrition_bcn, session_id, .keep_all = TRUE)$is_valid_vocab_size
)
...
nd <- expand_grid(
  distinct(data_bcn, condition, lp),
  age_std = 0,
  timebin_std = timebin_values
)
...
```{r}
tar_load(data_target)
tar_load(model_fits_target)

timebin_values <- make_std(
  seq(0, 16, length.out = 16),
  mean(data_target$timebin),
  sd(data_target$timebin)
)

voc_l1_std <- make_std(
  seq(0, 1, length.out = 3),
  mean(data_target$voc_l1),
  sd(data_target$voc_l1)
)

target_lv_values <- make_std(
  seq(0, 1, length.out = 3),
  mean(data_target$target_lv),
  sd(data_target$target_lv)
)

nd <- expand_grid(
  distinct(data_target, lp),
  voc_l1_std = 0,
  timebin_std = timebin_values,
  target_lv_std = target_lv_values
)

epreds <- add_epred_draws(
  nd,
  model_fits_target$fit_target_2,
  ndraws = NULL,
  re_formula = NA
)
...
obs <- data_target |>
  summarise(
    .elog = mean(.elog),
    .by = c(timebin_std, target_lv_std, lp)
  )
...
epreds_summary <- epreds |>
  ...
  summarise(
    ...
    .lower = mean_qi(.epred)[[2]],
    .upper = mean_qi(.epred)[[3]],
    .by = c(target_lv_std, timebin_std, lp)
  )
# mutate(
#   target_lv_std = factor(
#     target_lv_std,
#     levels = target_lv_values,
#     labels = scales::percent(seq(0, 1, length.out = 3)),
#     ordered = TRUE
#   )
# )

plot_timecourse <- epreds |>
  ggplot(aes(
    timebin_std,
    .epred,
    colour = target_lv_std,
    fill = target_lv_std,
    group = target_lv_std,
  )) +
  facet_wrap(~lp) +
  ...
  geom_point(
    data = obs,
    aes(x = timebin_std, y = .elog, colour = target_lv_std),
    stroke = 1,
    size = 1,
    shape = 1,
    alpha = 1 / 4,
  ...
  labs(
    x = "Time (ms)",
    y = "Logit (Target looking)",
    colour = "Target cognateness",
    fill = "Target cognateness",
    linetype = "Target cognateness",
    shape = "Target cognateness"
  ) +
...
epreds_voc <- add_epred_draws(
  ...
  summarise(
    ...
    .lower = mean_qi(.epred)[[2]],
    .upper = mean_qi(.epred)[[3]],
    .by = c(target_lv_std, timebin_std, lp, voc_l1_std)
  )

plot_heatmaps <- epreds_voc |>
  mutate(.value = as.numeric(.value)) |>
  ggplot(aes(timebin_std, target_lv_std, fill = .value)) +
  facet_grid(voc_l1_std~lp) +
  geom_raster() +
  scale_y_continuous() +
  labs(
    x = "Time (ms)",
    y = "Target cognateness",
    fill = "Logit(Target looking)",
  ) +
  scale_fill_distiller(palette = "Reds", direction = 1) +
  theme(legend.position = "bottom", strip.text = element_blank())

plot_timecourse / plot_heatmaps + plot_layout()
```
...
doe_mon_exp <- participants_bcn |>
  ...
  unnest_wider(doe) |>
  mutate(
    doe2 = ifelse(test_language == "Catalan", doe_spanish, doe_catalan),
    is_positive = doe2 > 0
  ) |>
  count(is_positive)
```

# Exploratory analysis: the effect of target word cognate status
...
oxf_vocab <- vocabulary |>
  ...
  inner_join(participants, by = join_by(child_id, session_id, vocab_id)) |>
...
tbl_data <- stimuli |>
  ...
  summarise(
    across(list_1:list_4, \(x) first(x, na_rm = TRUE)),
