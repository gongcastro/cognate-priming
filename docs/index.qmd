...
1. Valid prime: $\text{Prime}_{LT} \geq 750 \text{ms}$ (50%)
2. Valid test: $\text{Target}_{LT} + \text{Distractor}_{LT} \geq 1,000 \text{ms}$ (50%)
3. Minimum 100ms looking time to target, and minimum 100ms looking time to target (5%)

Participant-level inclusion criteria:

4. At least two valid trial of each type (cognate, non-cognate, unrelated)
...
polypoly::poly_add_columns(time_bin, degree = 3, prefix = "ot", scale_width = 1)
...
.by = c(id, lp, trial_type, age_group)) |> 
ggplot(aes(trial_type, 
.prop, 
colour = trial_type,
fill = trial_type)) +
...
outlier.colour = NA) +
labs(x = "Trial type",
y = "Proportion of target looking",
colour = "Trial type", 
fill = "Trial type") +
scale_y_continuous(labels = \(x) scales::percent(x, accuracy = 1)) +
...
.by = c(id, timebin, lp, trial_type, age_group)) |> 
...
shape = "Trial type") +
scale_y_continuous(labels = \(x) scales::percent(x, accuracy = 1)) +
scale_x_continuous(labels = \(x) format(x * 100, big.mark = ",")) +
theme(legend.position = "top",
legend.title = element_blank(),
...
#| fig-width: 12
#| fig-height: 8
nd <- expand_grid(timebin = seq(0, 20, 0.1),
