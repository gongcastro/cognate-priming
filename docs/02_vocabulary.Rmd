...
options(
	...
	ggplot2.discrete.slab_color = ggsci::pal_d3()(4),
...
sample_size <- participants %>% 
	filter(participant %in% fit_3$data$participant) %>% 
	count(age_group)

sample_size_lp <- participants %>% 
	filter(
		participant %in% fit_3$data$participant
	) %>% 
	count(age_group, lp) %>% 
	mutate(
		age_group = case_when(
...
participants %>% 
	...
	stat_slab(
		aes(side = ifelse(lp=="Monolingual", "left", "right")),
		colour = "white",
		alpha = 0.5
	) +
	geom_boxplot(
...
ggsave(here("img", "vocab_conceptual.png"))
